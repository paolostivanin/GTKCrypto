...
echo "--> Cheking deps..."
is_mud=$(find /usr/lib64/ -iname *mudflap* &>/dev/null && echo $?)
is_mud2=$(find /lib/ -iname *mudflap* &>/dev/null && echo $?)
is_gcry=$(find /usr/lib64/ -name libgcrypt.so &>/dev/null && echo $?)
is_gcry2=$(find /lib/ -name libgcrypt.so &>/dev/null && echo $?)
is_crypto=$(find /usr/lib64/ -name libcrypto.so &>/dev/null && echo $?)
