...
int compute_tiger(const char *filename){
	int algo, i, fd;
	char tigerhash[49];
	struct stat fileStat;
	char *buffer;
	const char *name = gcry_md_algo_name(GCRY_MD_TIGER);
	algo = gcry_md_map_name(name);
	off_t fsize = 0, donesize = 0, diff = 0;

	fd = open(filename, O_RDONLY | O_NOFOLLOW);
	if(fd == -1){
		printf("--> compute_tiger: failed to open file\n");
		return 1;
	}
	...
	nowhile:
	gcry_md_final(hd);
	unsigned char *tiger = gcry_md_read(hd, algo);
 	for(i=0; i<24; i++){
 		sprintf(tigerhash+(i*2), "%02x", tiger[i]);
 	}
 	tigerhash[48] = '\0';
 	printf("%s\n", tigerhash);
 	free(buffer);
