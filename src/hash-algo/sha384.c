...
int compute_sha384(const char *filename){
	int algo, i, fd;
	char sha384hash[97];
	struct stat fileStat;
	char *buffer;
	const char *name = gcry_md_algo_name(GCRY_MD_SHA384);
	algo = gcry_md_map_name(name);
	off_t fsize = 0, donesize = 0, diff = 0;

	fd = open(filename, O_RDONLY | O_NOFOLLOW);
	if(fd == -1){
		printf("--> compute_sha384: failed to open file\n");
		return 1;
	}
	...
	unsigned char *sha = gcry_md_read(hd, algo);
 	for(i=0; i<48; i++){
 		sprintf(sha384hash+(i*2), "%02x", sha[i]);
 	}
 	sha384hash[96] = '\0';
 	printf("%s\n", sha384hash);
 	free(buffer);
