...
GtkWidget *create_popover(GtkWidget *parent, GtkPositionType pos, struct main_vars *main_var) {
    ...
    for (i = 1, j = 0; i < 4; i++, j++)
        main_var->radio_button[i] = gtk_radio_button_new_with_label_from_widget(
                GTK_RADIO_BUTTON (main_var->radio_button[0]), algo[j]);

    main_var->radio_button[4] = gtk_radio_button_new_with_label_from_widget(NULL, "CBC");
    main_var->radio_button[5] = gtk_radio_button_new_with_label_from_widget(
            GTK_RADIO_BUTTON (main_var->radio_button[4]), "CTR");
