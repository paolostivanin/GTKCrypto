...
static void multiple_free(gboolean, gint, gpointer *, ...);

static void multiple_fclose(gint, FILE *, ...);

static void end_from_error(guint, struct main_vars *, const gchar *);


static void add_text(gpointer data, const gchar *text) {
    gtk_progress_bar_set_show_text(GTK_PROGRESS_BAR (data), TRUE);
    gtk_progress_bar_set_text(GTK_PROGRESS_BAR (data), text);
}


static gboolean bar_full(gpointer data) {
...
static gboolean bar_pulse(gpointer data) {
...
gpointer crypt_file(gpointer user_data) {
    ...
    if (fpout == NULL) {
        g_printerr("%s\n", strerror(errno));
        multiple_free(FALSE, 2, (gpointer *)&input_fname, (gpointer *)&output_fname);
        multiple_free(TRUE, 2, (gpointer *)&crypto_buffer, (gpointer *)&inputKey);
        fclose(fp);
...
static void multiple_free(gboolean gcrypt, gint argc, gpointer *buf, ...) {
    gint i = 0;
    va_list ptr;
    va_start(ptr, buf);
    while (i < argc) {
...
static void multiple_fclose(gint argc, FILE *fp, ...) {
    gint i = 0;
    va_list ptr;
    va_start(ptr, fp);
    while (i < argc) {
...
static void end_from_error(guint id, struct main_vars *main_var, const gchar *message) {
    g_source_remove(id);
