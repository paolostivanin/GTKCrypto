...
gpointer compute_md5(gpointer user_data) {
    ...
    else if (g_utf8_strlen(gtk_entry_get_text(GTK_ENTRY (hash_var->hash_entry[0])), -1) == MD5_DIGEST_SIZE*2)
        goto fine;
    ...
    g_close(fd, &err);
    gchar *hash = finalize_hash(hd, algo, MD5_DIGEST_SIZE);
    g_hash_table_insert(hash_var->hash_table, hash_var->key[0], g_strdup(hash));
    g_free(hash);

    fine:
    add_idle_and_check_id(id, hash_var, 0);
}
