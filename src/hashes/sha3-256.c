...
void *compute_sha3_256(struct hashWidget_t *HashWidget){
   	if(!gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(HashWidget->checkS3_256))){
		gtk_entry_set_text(GTK_ENTRY(HashWidget->entryS3_256), "");
		goto fine;
	}
	else if(strlen(gtk_entry_get_text(GTK_ENTRY(HashWidget->entryS3_256))) == 64){
		goto fine;
	}
	...
	if(fsize < BUF_FILE){
		...
		sha3_256_update(&ctx, fsize, fAddr);
	...
	while(fsize > donesize){
		...
		sha3_256_update(&ctx, BUF_FILE, fAddr);
		donesize+=BUF_FILE;
		diff=fsize-donesize;
		offset += BUF_FILE;
		if(diff < BUF_FILE && diff > 0){
			...
			sha3_256_update(&ctx, diff, fAddr);
	...
	sha3_256_digest(&ctx, SHA3_256_DIGEST_SIZE, digest);
 	...
 	hash[64] = '\0';
 	gtk_entry_set_text(GTK_ENTRY(HashWidget->entryS3_256), hash);
