...
void *compute_sha512(struct hashWidget_t *HashWidget){
	...
	while(fsize > donesize){
		...
		sha512_update(&ctx, BUF_FILE, fAddr);
		donesize+=BUF_FILE;
		diff=fsize-donesize;
		offset += BUF_FILE;
	...
	sha512_digest(&ctx, SHA512_DIGEST_SIZE, digest);
 	...
 	hash[128] = '\0';
 	gtk_entry_set_text(GTK_ENTRY(HashWidget->entryS512), hash);
